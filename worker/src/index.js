...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      if (!isAuthorized(request, env)) return corsJson(request, env, { error: "API key không đúng." }, 401);

      if (url.pathname === "/api/health" && request.method === "GET") {
        return corsJson(request, env, { ok: true, service: "shopee-price-watcher", time: new Date().toISOString() });
      }
      ...
      if (url.pathname === "/api/test-notification" && request.method === "POST") return testPush(request, env, ctx);

      const checkMatch = url.pathname.match(/^\/api\/products\/([^/]+)\/check$/);
      if (checkMatch && request.method === "POST") return checkOne(request, env, decodeURIComponent(checkMatch[1]), ctx);

      const productMatch = url.pathname.match(/^\/api\/products\/([^/]+)$/);
      if (productMatch && request.method === "DELETE") return deleteProduct(request, env, decodeURIComponent(productMatch[1]));
...
function allowedOrigin(request, env) {
  ...
  if (!configured || configured.includes("REPLACE_WITH")) return incoming || "*";
...
async function createProduct(request, env) {
  ...
  if (!rawUrl) return corsJson(request, env, { error: "Thiếu link sản phẩm Shopee." }, 400);

  const product = await fetchShopeeProduct(rawUrl, env);
  const id = `${product.shopId}:${product.itemId}`;
  const now = new Date().toISOString();
  const existing = await env.DB.prepare("SELECT id, active FROM products WHERE id = ?").bind(id).first();
  ...
  await env.DB.prepare(
    ...
    id, rawUrl, product.canonicalUrl, product.shopId, product.itemId, product.name, product.imageUrl,
    product.price, product.price, product.price, now, now
  ).run();
  await insertHistory(env, id, product.price, now);
  return corsJson(request, env, { ok: true, id, product: { ...product, baselinePrice: product.price } }, 201);
}
...
async function checkOne(request, env, id, ctx) {
  ...
  if (!product) return corsJson(request, env, { error: "Không tìm thấy sản phẩm." }, 404);
  const checked = await refreshProduct(env, product);
  if (checked.shouldNotify) ctx.waitUntil(notifyPriceDrop(env, checked.product));
  return corsJson(request, env, { ok: true, product: checked.product, notified: checked.shouldNotify });
}

async function checkAllProducts(env, scheduledTime) {
  const startedAt = new Date(scheduledTime || Date.now()).toISOString();
  const result = await env.DB.prepare("SELECT * FROM products WHERE active = 1 ORDER BY created_at ASC").all();
  ...
  for (const product of products) {
    try {
      ...
      await env.DB.prepare("UPDATE products SET checked_at = ?, check_error = ? WHERE id = ?")
        .bind(new Date().toISOString(), message, product.id).run();
      console.error(JSON.stringify({ event: "product_check_failed", productId: product.id, error: message }));
    }
  }
}

async function refreshProduct(env, row) {
  const fresh = await fetchShopeeProduct(row.canonical_url || row.url, env, { shopId: row.shop_id, itemId: row.item_id });
  const now = new Date().toISOString();
...
async function insertHistory(env, productId, price, checkedAt) {
  await env.DB.prepare("INSERT INTO price_history (product_id, price, checked_at) VALUES (?, ?, ?)")
...
async function fetchShopeeProduct(inputUrl, env, knownIds = null) {
  ...
  if (!ids?.shopId || !ids?.itemId) throw new Error("Không đọc được shop_id/item_id từ link Shopee này.");
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      if (payload?.error || payload?.error_msg) throw new Error(payload.error_msg || `Shopee error ${payload.error}`);
...
function parseShopeePayload(payload, ids, canonicalUrl) {
  ...
  const rawPrice = modelPrices.length ? Math.min(...modelPrices) : (priceCandidates.length ? Math.min(...priceCandidates) : null);
  if (!rawPrice) return null;

  const price = normalizeShopeePrice(rawPrice);
  if (!Number.isFinite(price) || price <= 0) return null;

  const imageValue = item.image || item.image_hash || item.images?.[0] || data?.product_images?.images?.[0] || data?.image;
  const imageUrl = imageValue
    ? (String(imageValue).startsWith("http") ? String(imageValue) : `https://down-vn.img.susercontent.com/file/${imageValue}`)
    : null;
...
function normalizeShopeePrice(raw) {
  // Shopee product-detail wire prices are represented in micros (currency value × 100000).
  return Math.round(Number(raw) / 100000);
}

function normalizeUrl(value) {
  let url;
  try { url = new URL(value); } catch { throw new Error("Link không hợp lệ."); }
  const host = url.hostname.toLowerCase();
  if (!host.endsWith("shopee.vn")) throw new Error("Hiện bản này chỉ nhận link Shopee Việt Nam.");
  return url.href;
}
...
async function resolveShopeeUrl(url) {
  ...
  const response = await fetch(url, {
    method: "GET",
    headers: { "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 18_5 like Mac OS X) AppleWebKit/605.1.15 Version/18.5 Mobile/15E148 Safari/604.1" },
    redirect: "follow"
  });
...
async function subscribePush(request, env) {
  ...
  if (!endpoint || !p256dh || !auth) return corsJson(request, env, { error: "Push subscription không hợp lệ." }, 400);
...
async function unsubscribePush(request, env) {
  const body = await request.json().catch(() => null);
  if (!body?.endpoint) return corsJson(request, env, { error: "Thiếu endpoint." }, 400);
  await env.DB.prepare("UPDATE push_subscriptions SET revoked_at = ? WHERE endpoint = ?")
    .bind(new Date().toISOString(), body.endpoint).run();
  return corsJson(request, env, { ok: true });
}

async function testPush(request, env, ctx) {
  ctx.waitUntil(sendPushToAll(env, {
...
async function notifyPriceDrop(env, product) {
  ...
  if (sent > 0) {
    await env.DB.prepare("UPDATE products SET last_notified_price = ? WHERE id = ?")
...
async function sendPushToAll(env, payload) {
  if (!env.VAPID_PUBLIC_KEY || !env.VAPID_PRIVATE_KEY || !env.VAPID_SUBJECT) {
    console.warn("VAPID secrets are not configured; skipping push.");
    return 0;
  }

  const result = await env.DB.prepare("SELECT endpoint, p256dh, auth FROM push_subscriptions WHERE revoked_at IS NULL").all();
  ...
  for (const sub of subscriptions) {
    try {
      ...
      if (response.ok) {
        ...
        console.warn(JSON.stringify({ event: "push_failed", status: response.status, endpoint: safeEndpoint(sub.endpoint) }));
      }
    } catch (error) {
